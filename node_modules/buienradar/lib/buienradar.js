...
(function () {
	...
	this.getRawRainData = function () {
		...
		const apiUrls = [
			...
			{
				host: 'br-gpsgadget-new.azurewebsites.net',
				path: `/data/raintext?lat=${this.lat}&lon=${this.lon}`,
			},
			{
		...
		return Promise.race(
			apiUrls.map((url, index, urlList) =>
				new Promise((res, rej) => {
					console.log('requesting data from', url.host + url.path);
					const req = http.request(url, (response) => {
						...
						response.on('end', () => {
							responseCounter++;
							if (data && data.match(rainDataRegex)) {
								res(data);
					...
					req.on('error', e => {
						responseCounter++;
						if (responseCounter === urlList.length) {
							rej(new Error('No url received rain data'));
						}
					});
					req.end();
